...
class music(commands.Cog):
    ...
    @app_commands.command(name="play",description="Toca uma miausica do YouTube.")
    @app_commands.describe(
        busca = "Digite o nome da miausica no YouTube"
    )
    async def play(self, interaction:discord.Interaction,busca:str):
        ...
        try:
            ...
            embedvc = discord.Embed(
                colour= 1646116,#grey
                description = 'Para tocar uma miausica, primeiro se conecte a um canal de voz.'
            )
            ...
            if type(song) == type(True):
                ...
                embedvc = discord.Embed(
                    colour= 32768,#green
                    description = f"Você adicionou a miausica **{song['title']}** à fila!"
                )
    ...
    @app_commands.command(name="fila",description="Mostra as atuais miausicas da fila.")
    async def q(self, interaction:discord.Interaction):
        ...
        if retval != "":
            ...
            embedvc = discord.Embed(
                colour= 1646116,
                description = 'Não existe miausicas na fila no momento.'
            )
            await interaction.followup.send(embed=embedvc)

    @app_commands.command(name="pular",description="Pula a atual miausica que está tocando.")
    ...
    async def pular(self, interaction:discord.Interaction):
        ...
        if self.vc != "" and self.vc:
            ...
            embedvc = discord.Embed(
                ...
                description = f"Você pulou a miausica."
            )
            await interaction.followup.send(embed=embedvc)
    ...
    async def skip_error(self,interaction: discord.Interaction, error: app_commands.AppCommandError):
        if isinstance(error, commands.MissingPermissions):
            embedvc = discord.Embed(
                colour= 12255232,
                description = f"Você precisa da permissão **Gerenciar canais** para pular miausicas."
            )
            await interaction.followup.send(embed=embedvc)     
        else:
            raise error
